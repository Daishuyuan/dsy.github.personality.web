...
legacyPath: "/2018/03/12/Zero-Shot Learning/"
toc: true
tags:
    - 论文
    - python
...
**定义 1** （Zero-Shot Classification, ZSC）.给定一个对象类别集合<img src="https://latex.codecogs.com/gif.latex?  C=\{c_1,c_2,...,c_n\} "/>，这个集合的训练集合为<img src="https://latex.codecogs.com/gif.latex? Y = \{y_1,y_2,...,y_k\}"/>，和类别集合的测试集合<img src="https://latex.codecogs.com/gif.latex? Z=\{z_1,z_2,...,z_l\}"/>，当<img src="https://latex.codecogs.com/gif.latex? Y \cap Z = \varnothing, and Y \cup Z = C"/>。一个训练集合包含Y中被标记的样本，但是不包含Z中被标记的样本。这种分类任务叫做ZSC。
...
**定义2** （Attribute Annotation of Object Classes）。给定一个对象类别的集合<img src="https://latex.codecogs.com/gif.latex? C=\{c_1,c_2,...,c_n\}"/>和属性集合<img src="https://latex.codecogs.com/gif.latex? A=\{a_1,a_2,...,a_m\}"/>,一个对象的类别<img src="https://latex.codecogs.com/gif.latex? c_j \in C"/>可以被描述为一个m维度的向量<img src="https://latex.codecogs.com/gif.latex? V_j = \{v_{j1},v_{j2},...,v_{jm}\}(v_{ji} \in [0,1], 1 \le i \le m)"/>值<img src="https://latex.codecogs.com/gif.latex? v_{ji}"/>在向量<img src="https://latex.codecogs.com/gif.latex? \mathrm v_j"/>代表相对相关强度在对象类别<img src="https://latex.codecogs.com/gif.latex? c_j"/>和属性<img src="https://latex.codecogs.com/gif.latex? a_i"/>中。使<img src="https://latex.codecogs.com/gif.latex? V = (v_1,v_2,...,v_n)^\mathrm T"/>以及<img src="https://latex.codecogs.com/gif.latex? C=(c_1,c_2,...,c_n)^\mathrm T"/>然后用<img src="https://latex.codecogs.com/gif.latex? (\mathrm V, \mathrm C)"/>代表对象类别的属性标签。

&emsp;&emsp;使<img src="https://latex.codecogs.com/gif.latex? F_{train}"/>成为由<img src="https://latex.codecogs.com/gif.latex? \alpha-dimensional"/>从训练图片中抽取的低等级图片的特征组成的矩阵。在DAP中，属性预测器可以使用<img src="https://latex.codecogs.com/gif.latex? (\alpha + 1)-dimensional"/>的训练数据<img src="https://latex.codecogs.com/gif.latex? (F_{train},Y_{train})"/>和属性注解<img src="https://latex.codecogs.com/gif.latex? \mathrm v"/>。对于一个属性，<img src="https://latex.codecogs.com/gif.latex? a_i \in A(1 \le i \le m)"/>，训练数据<img src="https://latex.codecogs.com/gif.latex? (F_{train},V_{train}^i)"/>，构建<img src="https://latex.codecogs.com/gif.latex? a_i^{'}S"/> 预测器可以从 <img src="https://latex.codecogs.com/gif.latex? Y_{train} \text{ with } v_{ji}"/>替换得到（<img src="https://latex.codecogs.com/gif.latex? v_{ji} \text{ with } y_j \text{ and } a_i"/>是相对关联强度）。方法<img src="https://latex.codecogs.com/gif.latex? fun_{a_i}:f \to v (f \in R^\alpha,v \in [0,1])"/>以SVM<img src="https://latex.codecogs.com/gif.latex? (F_{train},v_{train}^i)"/>构建，作为<img src="https://latex.codecogs.com/gif.latex? a_i"/>的预测器。然而，一个对象的<img src="https://latex.codecogs.com/gif.latex? a_i"/>值可以使用<img src="https://latex.codecogs.com/gif.latex? fun_{a_i}"/>从<img src="https://latex.codecogs.com/gif.latex? \alpha-dimensinal"/>预测。使用CORL的属性预测器即可从上述过程中构建。
...
___
**Input:** Taxonomy taxo, DataSet ds,Root cls<img src="https://latex.codecogs.com/gif.latex? \leftarrow"/>"root"
**Output:** RuleSet rs <img src="https://latex.codecogs.com/gif.latex? \leftarrow \varnothing"/>

1. <img src="https://latex.codecogs.com/gif.latex? tmpDS \leftarrow copy(ds),sub \leftarrow getSubCls(taxo,cls);"/> //create a copy of ds & get direct subclasses of cls
...
2. **for each** <img src="https://latex.codecogs.com/gif.latex? \text{inst} \in tmpDS"/> **do**
3. &emsp;&emsp;&emsp;&emsp;<img src="https://latex.codecogs.com/gif.latex? lbl_{set} \leftarrow subs \cap 4. getSuperClasses(taxo,labelOf(inst));"/> // obtain super-class of inst's label in subs
4. &emsp;&emsp;&emsp;&emsp;**if** <img src="https://latex.codecogs.com/gif.latex? lbl_{set} = \varnothing"/> **then** <img src="https://latex.codecogs.com/gif.latex? inst.label \leftarrow getAnElement(lbl_{set});"/> // replace label guided by the hierachy
5. &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;**else** <img src="https://latex.codecogs.com/gif.latex? tmpDS \leftarrow tmpDS_{inst}"/>
6. &emsp;&emsp;&emsp;&emsp;**end if**
7. **end for**
8. <img src="https://latex.codecogs.com/gif.latex? attrs_{set} \leftarrow \varnothing, ml_rules \leftarrow \varnothing"/>;
9. **repeat** /\*step 9-12：learning rules with different attributes/\*
10. <img src="https://latex.codecogs.com/gif.latex? attrs_{set} \leftarrow attrs_{set} \cup attributesInRules(ml_{rules})"/> \\exclude attributes already in rules
11. <img src="https://latex.codecogs.com/gif.latex? ml_rules \leftarrow CR_{learning}(tmpDS,attrs_{set})"/>//learning rules without using attributes in attrs_set
12. **until** <img src="https://latex.codecogs.com/gif.latex? ml_{rules} = \varnothing"/>//some rules are learned using new attributes
/\* step 13-16: form HCRs deriving cls's subclass(r's head)from cls\*/
13. **for each** <img src="https://latex.codecogs.com/gif.latex? r \in ml_{rules}"/> **do**
14. <img src="https://latex.codecogs.com/gif.latex? hcr \leftarrow formHCR(r,cls)"/>; // add class = cls to the antecedents of r to form the HCR
15. <img src="https://latex.codecogs.com/gif.latex? rs \leftarrow rs \cup \{hcr\}"/>；//add onto_rule to the rules set rs
16. **end for**
/\*steps 17-20: hierachical learning: learning rules for cls's subclasses\*/
17. <img src="https://latex.codecogs.com/gif.latex? tmpDS1 \leftarrow copy(ds)"/>；//create a new copy of ds for learning
18. **for each** <img src="https://latex.codecogs.com/gif.latex? sub \in subs"/> **do**
19. &emsp;&emsp;&emsp;&emsp;<img src="https://latex.codecogs.com/gif.latex? rs \leftarrow rs \cup OntoGuided_{Rule}^{Learning}(taxo,tmpDS1,sub)"/>；
20. **end for**
